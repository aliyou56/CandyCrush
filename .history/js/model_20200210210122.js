...
class Model {
    ...
    constructor(gridSize_) {
        ...
        this.grid = this.init()
        // console.log(this.possibleExplodeArray())
    }
    ...
    explode() {
        console.log("[Model.explode]: I make an explosion")
        var arrToBeExplode = this.possibleExplodeArray()
        if(arrToBeExplode.length > 0) {
            ...
            console.log("[Model.explode]: result -> ", table(this.grid))
    ...
    isExplodePossible() {
        ...
        for(let i=0; i<this.gridSize; i++) {
            ...
            for(let j=0; j<(this.gridSize-2); j++) {
                ...
                while(  k1<this.gridSize && this.grid[i][j] === this.grid[i][k1] ) {
                    k1++; counterHorizontal++
                }
                while( k2<this.gridSize && this.grid[j][i] === this.grid[k2][i] ) {
                    k2++; counterVertical++
                }
    ...
    possibleExplodeArray() {
        console.log("Model -> possibleExplodeArray : je fourni la liste des alignements")
        var result = []
        for(let i=0; i<this.gridSize; i++) {
            // horizontal
            for(let j=0; j<(this.gridSize-2); j++) {
        ...
        console.log("Model -> possibleExplodeArray : ", table(result))
        return result
    }
}