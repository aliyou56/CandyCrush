...
class Controller {
   ...
   gameEventHandler(context_) {
        ...
        if(this.model.isAlignmentExist()) {
            ...
            if(isGridNotFull) {
                that.repackGrid(context_)
                setTimeout(() => {that.view.animate(context_, that.gameEventHandler)}, 0)
                //setTimethis.view.animate(context_, that.gameEventHandler)
            }
