...
function init() { // démarrage du jeu
    ...
    var game = new Controller(10, 50) // cree une grille de 10x10 avec des lutins de taille 50x50
    // le controleur cree le modele et la vue
    game.view.init(); 
    game.view.anime(context)
}
...
class Sprite {
    ...
    draw(context) {
        ...
        // +"board("+this.x+", "+this.y+"), to("+this.toX+", "+this.toX+"), ")
        var that = this
        var img = new Image()
        img.onload = function() {
            if(that.isSelected) {
...
// 
class Controller {
    constructor(boardSize_, spriteSize_) {
        this.boardSize = boardSize_
        this.model = new Model()
        this.view = new View(this.model, spriteSize_)
        this.view.init()
    }
}