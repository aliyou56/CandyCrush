...
class Sprite {
    ...
    constructor(obj_, width_, height_) {
        ...
        this.x = this.toX = 0
        this.y = this.toY = 0
        this.pageX = document.getElementById("canvas").offsetLeft
        this.pageY = document.getElementById("canvas").offsetTop
    }
    ...
    position(x_, y_) {
        this.x = this.toX = x_
        this.y = this.toY = y_
        this.pageX = document.getElementById("canvas").offsetLeft + (x_ * 40)
        this.pageY = document.getElementById("canvas").offsetTop + (y_ * 40)
    }

    // Draw the sprite to it's current position in the given context.
    // TODO when isSelected
    draw(context) {
         console.log("in Sprite->draw: obj="+this.obj+" - p("+this.x+", "+this.y
        ...
        img.onload = function() {
            if(this.isSelected) {
                ...
                context.drawImage(img, this.pageX, this.pageY, this.width, this.height);
            }
        }
    ...
    update() {
        ...
        // console.log("in Sprite->update: obj="+this.obj+" - p("+this.x+", "+this.y
        // +"), to("+this.toX+", "+this.toY+"), page("+this.pageX+", "+this.pageY
        // +") toPage("+toPageX+","+toPageY+")")
        ...
        // console.log("ouSprite->update: obj="+this.obj+" - p("+this.x+", "+this.y
        // +"), to("+this.toX+", "+this.toY+"), page("+this.pageX+", "+this.pageY
        // +") toPage("+toPageX+","+toPageY+")\n")
    }
